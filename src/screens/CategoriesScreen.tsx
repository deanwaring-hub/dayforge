    ...
    >
      ...
      >
        ...
        <View style={cs.sheetHeader}>
          ...
          >
            <Text style={[cs.closeBtnText, { color: theme.colors.textMuted }]}>
              ✕
            </Text>
          </TouchableOpacity>
        </View>
        ...
        >
          ...
          >
            ...
            <Text
              style={[
                cs.previewLabel,
                { color: form.color, fontFamily: theme.fonts.body },
              ]}
            >
              {form.label || "Preview"}
            </Text>
          ...
          <View style={[cs.segmentRow, { marginBottom: 16 }]}>
            {(["high", "normal", "low"] as const).map((t) => {
              ...
              return (
                <TouchableOpacity
                  key={t}
                  onPress={() => update({ defaultPriorityTier: t })}
                  style={[
                    cs.segmentBtn,
                    {
                      backgroundColor: isSelected
          ...
          <View style={[cs.chipRow, { marginBottom: 16 }]}>
            {DURATION_OPTIONS.map((d) => (
              <TouchableOpacity
                ...
                accessibilityRole="radio"
                accessibilityState={{ checked: form.defaultDuration === d }}
              >
                <Text
                  style={[
                    cs.chipText,
          ...
          <View style={[cs.chipRow, { marginBottom: 16 }]}>
            {[0, 5, 10, 15, 30].map((b) => (
              <TouchableOpacity
                key={b}
                onPress={() => update({ defaultBufferAfter: b })}
                style={[
                  cs.chip,
                  {
                    backgroundColor:
                      form.defaultBufferAfter === b
                        ? theme.colors.accent
                        : theme.colors.surfaceAlt,
                    borderColor:
                      form.defaultBufferAfter === b
                ...
                accessibilityRole="radio"
                accessibilityState={{ checked: form.defaultBufferAfter === b }}
              >
                <Text
                  style={[
                    cs.chipText,
...
export default function CategoriesScreen({ onClose }: Props) {
  ...
  return (
    <SafeAreaView
      style={[cs.screen, { backgroundColor: theme.colors.background }]}
    >
      {/* Header */}
        ...
        {categories.map((cat) => (
          ...
          >
            ...
            <View style={cs.categoryInfo}>
              ...
              >
                {cat.defaultPriority} ·{" "}
                {cat.defaultDuration >= 60
                  ? `${cat.defaultDuration / 60}h`
                  : `${cat.defaultDuration}m`}{" "}
                · {cat.defaultNotificationEnabled ? "🔔" : "🔕"}
              </Text>
            </View>
            <TouchableOpacity
