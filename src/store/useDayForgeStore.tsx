...
} from "../database/queries/recurrenceQueries";
import {
  createInstance,
  ...
  getInstancesForDate,
  type TaskInstance,
} from "../database/queries/instanceQueries";
import {
  calculateAndSaveScore,
  getScoreForDate,
  type DailyScore,
} from "../database/queries/scoreQueries";
...
type Action =
  ...
  | {
      type: "SET_SCHEDULE";
      payload: { schedule: ScheduledTask[]; score: DailyScore | null };
    }
  ...
  switch (action.type) {
    ...
    case "SET_SCHEDULE":
      return {
        ...
        todayScore: action.payload.score,
          ...
          st.instance.id === action.payload.instanceId
            ? {
                ...
                instance: {
                  ...
                  snoozeCount:
                    action.payload.snoozeCount ?? st.instance.snoozeCount,
                },
              }
            : st,
        ),
...
function buildScheduleAlgo(
  ...
  const fixed = occurring
    ...
    .sort(
      (a, b) =>
        timeToMinutes(a.time || "00:00") - timeToMinutes(b.time || "00:00"),
    );
  ...
  const tryPlace = (
    ...
    scheduled.push({
      ...
      displayEnd: preferredStart + task.duration,
    });
    return true;
  };
  ...
  for (const task of fixed) {
    ...
    if (overlaps(blockStart, blockEnd)) {
      ...
      const conflicting = scheduled.find(
        (s) => !(blockEnd <= s.startMins || blockStart >= s.endMins),
      );
      ...
      scheduled.push({
        ...
        startMins: blockStart,
        endMins: blockEnd,
        displayStart: timeMins,
        displayEnd: timeMins + task.duration,
      });
    } else {
      scheduled.push({
        task,
  ...
  for (const task of flexible) {
    if (completedTaskIds.has(task.id)) continue;
    const rule = rules.get(task.id)!;
    const rawPreferred = task.preferredTime
      ? timeToMinutes(task.preferredTime)
      : effectiveStartMins;
    const preferred = Math.max(rawPreferred, effectiveStartMins);
    const rawEarliest = task.earliestStart
      ? timeToMinutes(task.earliestStart)
      : dayStartMins;
    const earliest = Math.max(rawEarliest, effectiveStartMins);
    const latest = task.latestEnd
      ? timeToMinutes(task.latestEnd) - task.duration
      : dayEndMins - task.duration;
    let placed = false;
    for (let offset = 0; offset <= 120; offset += 15) {
      if (
        preferred + offset <= latest &&
        preferred + offset >= earliest &&
        tryPlace(task, rule, preferred + offset)
      ) {
        placed = true;
        break;
      }
...
type StoreContextType = State & {
  ...
  editTask: (id: string, patch: Partial<CreateTaskData>, recurrence?: Partial<RecurrenceRule>) => Promise<void>;
...
export function DayForgeProvider({ children }: { children: React.ReactNode }) {
  ...
  const buildScheduleFromData = useCallback(
    async (tasks: Task[], categories: Category[], user: User) => {
      ...
      dispatch({
        type: "SET_SCHEDULE",
        payload: { schedule: scheduledTasks, score },
      });
    },
    [],
  );
  ...
  const initialise = useCallback(async () => {
    ...
    try {
      const [user, categories, tasks] = await Promise.all([
        getUser(),
        getCategories(),
        getAllTasks(),
      ]);
  ...
  const finishOnboarding = useCallback(
    async (settings: { dayStart: string; dayEnd: string; theme: string }) => {
      await completeOnboarding(settings);
      const user = await getUser();
      dispatch({ type: "SET_USER", payload: user });
      dispatch({ type: "SET_ONBOARDED", payload: true });
      if (user)
        await buildScheduleFromData(
  ...
  // Updated to accept full category data including defaults
  ...
  const editCategory = useCallback(
    async (id: string, patch: Partial<Omit<Category, "id" | "createdAt">>) => {
  ...
  const editTask = useCallback(
    async (
      ...
      if (recurrence) {
        await updateRecurrenceRule(id, recurrence);
      }
      const tasks = await getAllTasks();
      dispatch({ type: "SET_TASKS", payload: tasks });
      const { categories, user } = stateRef.current;
  ...
  const pauseTaskUntil = useCallback(
    async (id: string, pausedUntil: string) => {
  ...
  const resumeTaskNow = useCallback(
    async (id: string) => {
      await resumeTask(id);
      const tasks = await getAllTasks();
      dispatch({ type: "SET_TASKS", payload: tasks });
  ...
  const uncompleteTask = useCallback(async (instanceId: string) => {
    ...
    dispatch({
      type: "UPDATE_INSTANCE",
      payload: { instanceId, status: "pending", completedAt: undefined },
    });
    const score = await calculateAndSaveScore(stateRef.current.todayDate);
    dispatch({ type: "SET_SCORE", payload: score });
  }, []);
  ...
  const skipTask = useCallback(async (instanceId: string) => {
    ...
    dispatch({
      type: "UPDATE_INSTANCE",
      payload: { instanceId, status: "skipped" },
    });
    const score = await calculateAndSaveScore(stateRef.current.todayDate);
    dispatch({ type: "SET_SCORE", payload: score });
  }, []);
...
export function useDayForgeStore() {
  ...
  if (!context)
    throw new Error("useDayForgeStore must be used within a DayForgeProvider");
  return context;
}
